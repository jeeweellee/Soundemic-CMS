<?php
    ...
    */
    ...
    $genre = filter_input(INPUT_POST, 'genreId', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        ...
        && strlen($_POST['artist']) > 1 && isset($_POST['genre'])) {      
            ...
            $query = "INSERT INTO songs (title, description, artist, genre) VALUES (:title, :description, :artist, :genre)";
    ...
    function update() {
        ...
        if ($_POST && strlen($_POST['title']) && strlen($_POST['description']) && strlen($_POST['artist']) && isset($_POST['genre']) && strlen($_POST['songId'])) {
            ...
            // Builds the SQL query and binds it above the sanitized values.
            $query = "UPDATE songs SET title = :title, description = :description, artist = :artist, genre = :genre WHERE songId = :songId";
            $statement = $db->prepare($query);
